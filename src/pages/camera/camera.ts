...
export class CameraPage {
  ...
  onUpload(): void {
    ...
    let serverURL: string = 'http://homologa.api.drclube.com/api/clients/picture';
    ...
    fileTransfer.upload(this.photo.nativeURL, `${serverURL}`, options)
  ...
  private correctPathAndGetFileName(fileUri: string, sourceType: number): Promise<{oldFilePath: string, oldFileName: string}> {
    ...
    if (this.platform.is('android') && sourceType === this.camera.PictureSourceType.PHOTOLIBRARY) {
      ...
      return <Promise<{oldFilePath: string, oldFileName: string}>>this.filePath.resolveNativePath(fileUri)
  ...
  private saveFile(fileUri: string, sourceType: number): Promise<Entry> {
    return <Promise<Entry>>this.correctPathAndGetFileName(fileUri, sourceType)
